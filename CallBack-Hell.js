...
function gerRollNo() {
  //is tara shep ko pyramid of doom bolte hai is avoided Call-Back Hell ?
  ...
  setTimeout(() => {
    ...
    setTimeout(
      (rollnumber) => {
        ...
        console.log(
          `My name is ${biodata.name} and I an ${biodata.age} years old My ${rollnumber} Number`
        );
        ...
        setTimeout(
          (name) => {
            biodata.gender = "Male";
            console.log(
              `My name is ${biodata.name} Gender is ${biodata.gender}`
            );
            // console.log(name);
          },
          2000,
      ...
      2000,
      roll_no[1]
    ); //2 milge ga us array ki index se accessing kar re hai ?
  }, 2000);
}
...
function calllFunction() {
  ...
  function getCheese(callback) {
    setTimeout(() => {
      //1mili second ye cheese function call hoga ?
      let cheese = " 🧀 ";
  ...
  function makePizza(dough1, callback1) {
    //pizza ko hona dough pizza banane ke liye ?
    setTimeout(() => {
      let pizza = dough1 + " 🍕";
  ...
  getCheese((cheeseAccess) => {
    //92 line se ye cheese milra hai us ke parameter se ?
    console.log("Received cheese:", cheeseAccess);
    makeDough(cheeseAccess, (AccessDough) => {
      //ye cheeseAcesse line 100 parameter se mil ra hai arugument me kuch bhi naam pass kar sakte hai agrument me ham cheeseAccess function ko hi lare re hai q ke parametar me hamne cheese1 pass kiya hai is liye hame argument me kuch tu lena hoga is liye ham cheeseAccess hi lere hai argument me?
